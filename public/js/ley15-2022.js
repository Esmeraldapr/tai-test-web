// ============================================================
// Lógica de "Ley 15/2022, igualdad de trato" (ley15-2022.html)
// Pinta el texto (Títulos → Capítulos → Artículos, Disposiciones) a partir
// de data/ley15-2022.json, con lector de voz en cola, y la marca de en qué
// exámenes oficiales ha caído cada artículo.
// ============================================================

// ---------------- 1) Qué entra ----------------
// Confirmada por su nombre exacto en el código oficial del BOE (nº 21 de 51
// normas). Va con el resto de leyes de igualdad y no discriminación
// (LO 3/2007, RDL 1/2013...) del tema 5.
const TITULOS_QUE_ENTRAN = {
  "tit-preliminar": true, "tit-i": true, "tit-ii": true,
  "tit-iii": true, "tit-iv": true, "tit-v": true,
};
...
// CERO apariciones por ahora. Entra por estar en el código oficial del BOE,
// pero nunca ha caído todavía.
...
(async function iniciar() {
  ...
  let datos;
  try {
    const res = await fetch("data/ley15-2022.json");
    if (!res.ok) throw new Error("HTTP " + res.status);
    datos = await res.json();
  } catch (e) {
    console.error("Error cargando ley15-2022.json:", e);
